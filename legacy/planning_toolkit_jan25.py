...
def get_year_weeks_matrix(year, underflow=0, overflow=0):
    ...
    previous_year_start = iso_year_start_monday(year - 1)
    ...
    previous_total_iso_weeks = (year_start - previous_year_start).days // 7
    ...
    for i in range(previous_total_iso_weeks-underflow+1, previous_total_iso_weeks+1):  # e.g. -2, -1 if underflow=2
        monday = previous_year_start + datetime.timedelta(days=7 * (i - 1))
...
for week in date_matrix:
    ...
    for date in week['dates']:
        ...
        _cell.value = date
        _cell.number_format = 'mmm-dd'
        # format the date cell as a date month-date
...
# test:
# change style properties of a cell
double = Side(border_style="double", color="00ff0000")
_cell.border = Border(top=double, bottom=double)
